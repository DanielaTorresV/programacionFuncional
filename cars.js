...
function ferrarisArray (arr){
    const filterArray = arr.filter(function(car){
        return car.brand === "Ferrari";
    });
    return filterArray;
};
console.log(ferrarisArray(cars));

//2. Create a function nineties that returns all the cars with models between 1980 and 1990.

function nineties (arr){
    const modelArray = arr.filter(function(car){
        return car.model <= 1990 && car.model >= 1980;
    });
    return modelArray;
};
console.log(nineties(cars));

/*3. Create a function list that returns an array of strings with 
the same number of cars. Each element should contain the string
 “This from costs $” (e.g. for the first element it should say 
    “This Mazda from 1989 costs $5241”).*/

function list (arr){
    const newArray = arr.map (function(car){
...
function bmwSum(arr){
    ...
    const mapArray = filterArray.map(function(car){
        return car.price
    });

    const sumTotal = mapArray.reduce(function (a, b){
        return a+b;
    });
    return sumTotal;
};
